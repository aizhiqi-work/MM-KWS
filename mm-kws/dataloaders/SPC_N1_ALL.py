...
class SPC_N1_Dataset(Dataset):
    def __init__(
            ...
            save_path="/nvme01/aizq/mmkws/mmkws_submits/spc/SPC1/SPC_N1_ALL.csv",
        ...
        if preprocess:
            ...
            for wav_idx in range(len(wav_id)):
                ...
                if query_text in Targets:
                    for comparison_text in Targets:
                        support_wav = random.choices(supports_wavs[comparison_text], k=2)
                    ...
                    for comparison_text in Targets:
                        support_wav = random.choices(supports_wavs[comparison_text], k=5)
                        _label = 0
                        target_dict[idx] = {
                            'id': wav_id[wav_idx],
                            'Query_text': query_text,
