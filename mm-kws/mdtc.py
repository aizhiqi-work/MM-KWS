...
class MDTC_KWS(nn.Module):
    def __init__(
        ...
        self.kws_model =  MDTC(4, 4, 64, 64, 5, causal=True)
        self.output = Linear(encoder_dim, output_dim)
    
    def forward(self, inputs: Tensor, input_lengths: Tensor) -> Tuple[Tensor, Tensor]:
        outputs, output_lengths = self.conv_subsample(inputs, input_lengths)
        outputs = self.input_projection(outputs)
        outputs, _ = self.kws_model(outputs)
        return self.output(outputs), output_lengths
...
if __name__ == '__main__':
    ...
    x = torch.randn(128, 182, 80)  # batch-size * time * dim
    input_lengths = torch.tensor([124, 125] * 64)
    print(input_lengths)
    y, _ = mdtc(x, input_lengths)
