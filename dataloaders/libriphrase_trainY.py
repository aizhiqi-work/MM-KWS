...
class LibriPhrase_Train_Dataset(Dataset):
    def __init__(
        ...
        audiolm_env="/nvme01/aizq/mmkws/datasets/LibriPhrase_Train_MIN_20/audiolm",
        ...
        self.data_keys = list(self.keys[0].keys())
        self.all_data_keys = list(self.all_data[0].keys())
    ...
    def get_mini_batch(self, key, matching_words):
        ...
        neg_keys_MAX = random.choices(self.data_keys, k=15)
        neg_keys_MIN = random.choices(self.all_data_keys, k=15)
        for match in matching_words:
            match = match[0]
            pos_keys.extend([f'{match}__{i}' for i in range(len(self.keys[0][match]))])
            if match in neg_keys_MAX:
                neg_keys_MAX.remove(match)
            if match in neg_keys_MIN:
                neg_keys_MIN.remove(match)
        for word in mini_support_keys: 
            if word in pos_keys:
                pos_keys.remove(word)
        mini_batch_pos = random.choices(pos_keys, k=15)
        random_nei_keys = []
        random_nei_keys.extend(random.choices(neg_keys_MAX, k=3))
        random_nei_keys.extend(random.choices(neg_keys_MIN, k=2))
        mini_batch_neg = []
        for _key in random_nei_keys:
            mini_batch_neg.extend(random.choices([f'{_key}__{i}' for i in range(len(self.all_data[0][_key]))], k=1))
        ...
        _mini_batch_neg_wavs = [fbank(self.key2wav_hard(neg), num_mel_bins=80) for neg in mini_batch_neg]
    ...
    def key2audiolm(self, key):
        audio_data = self.audiolm_txn.get(key.encode())
