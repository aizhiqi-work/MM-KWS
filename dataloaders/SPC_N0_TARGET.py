...
class SPC_NO_Dataset(Dataset):
    def __init__(
        ...
        if preprocess:
            target_dict = {}
            idx = 0
            self.data = pd.DataFrame(columns=['id', 'Query_text', 'Query_wav', 'Support_text', 'Query_label', 'Support_label', 'label'])
    ...
    def __getitem__(
        ...
        ids, Query_text, Query_wav, Support_text, Query_label, Support_label, label = self.data[index]
        ...
        return ids, fbank_feature, g2p_embed, torch.from_numpy(lm_embed).squeeze(0), Query_label, Support_label, torch.tensor(label)
...
def collate_fn(batch):
    ids, fbank_feature, g2p_embed, lm_embed, Query_label, Support_label, label = zip(*batch)
    ...
    return ids, padded_fbank_feature, torch.tensor(lengths), padded_g2p_embed, padded_lm_embed, Query_label, Support_label, label_tensor
